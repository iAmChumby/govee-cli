...
The console's existing "optical bench" language — hairline borders, zero shadows outside
`Dialog`/`Popover`, flat surfaces, mono micro-labels, spring-driven glow — stays the
system for v3. New surfaces extend it with the existing token set
(`webui/app/src/styles/tokens.css`); nothing here introduces a new token unless noted.

- **Active Mode badge** (new, small component, `components/device/active-mode-badge.tsx`
  or inline in `control-deck.tsx`'s `ReadoutStrip`): mono `font-mono` text at the
  existing `--tracking-micro` (0.14em), colored by confidence — `confirmed` uses
  `--text-hi`, `assumed` uses `--text-mid`, `external` uses `--sage` (the one existing
  "attention, but not alarm" tone), `unknown` uses `--text-low`. No new color tokens. Age
  ("3h ago") renders in `--text-low` beside the label, same visual weight as
  `SegmentRail`'s existing caption. The "not what I see" reset button is an `IconButton`
  (`ghost` variant, existing primitive) using the existing hand-drawn glyph set idiom
  (`glyphs.tsx`) — add one new glyph, `ResetGlyph`, matching the stroke weight/style of
  `PowerGlyph`/`FilamentMark`.
- **Motion canvas layer**: no chrome of its own — it *is* the lamp's emitting surface,
  mounted between `CYLINDER_SHADING` and `Halo` inside the existing instrument shell. It
  inherits the "flashiness lives where a real lamp's light show would live" principle —
  the surrounding UI chrome stays exactly as flat/hairline as today.
- **Paint Studio**: `canvas-grid.tsx` sits inside a `Panel` (`raised` variant, matching
  `Dial`'s and `Slider`'s existing container). Tool buttons reuse `IconButton`
  (`sm`/`ghost`) in a row; the active tool gets the same `--glow-alpha`-driven ring
  `Switch` uses for its pending state, not a new selected-state treatment. Palette swatch
  strip reuses `SwatchRow` verbatim. The "approximate mapping — calibrate" banner uses the
  `Chip` primitive, `warn` tone (`--ember` family), not a bespoke banner component. The
  dual-preview split uses two `Panel`s (`flat`) side by side on desktop, stacked with a
  `Tabs`-style switcher (reusing `ui/tabs.tsx`'s shared-`layoutId` underline) on mobile
  widths.
- **Calibration wizard**: a `Dialog` (center-modal variant, `springHeavy` entrance,
  matching every other multi-step flow like `AddDeviceDialog`). The 15-chip drag-reorder
  strip uses `Chip` primitives at `radius-chip`, draggable via the same Pointer Events
  pattern as `canvas-grid.tsx` rather than a drag library.
- **Schedules timeline**: `Panel` (`flat`) container, `--hairline` tick marks at each
  hour, `--hairline-strong` at 00/06/12/18. Native chips: `accent`-colored,
  `radius-chip`. wake-ramp band: `--accent-dim` fill when armed/will-run, hairline-only
  outline (no fill) when not — reusing the existing "off" visual language `Switch`
  already uses for its unchecked track. Generic cron markers: `--text-low` dashed circles,
  `radius: 999px` (a new tiny inline style, not a new token — no radius token this small
  exists and one isn't needed elsewhere). Now-marker: 1px `--accent` line, no glow.
- **External Automation panel rows**: dashed border using `--hairline-strong` at reduced
  opacity via an inline `border-style: dashed` (first dashed-border usage in the system —
  acceptable as the one deliberate visual signal for "not editable here," matching this
  section's explicit design requirement that external rows never look native).
- **Scheduler health breakdown** (`ConnectionSection`): reuses the existing
  label+value row pattern already in that file, `StatusDot` for `native.alive` and a
  second `StatusDot` for `external.crontab_readable`, both using the existing
  `StatusTone` enum (`ok`/`warn`/`off`) — no new dot variants.
- **Typography/motion**: no new font, no new spring config beyond `springStandard`/
  `springHeavy`/`fadeFast` already in `lib/motion.ts` — the calibration wizard's chip
  reordering and the timeline's chip entrances both reuse `springStandard`. The one new
  named spring worth adding, if the warmth-spring duplication (stage-and-design-system
  hazard: `useWarmth`'s bespoke `{stiffness:46, damping:18}` inline literal) is touched
  incidentally by the motion-engine integration task, is `springWarmup` promoted into
  `lib/motion.ts` — optional, not required for this spec's scope, but doing it is free if
  that file is already open for the `MotionCanvas` integration.

---

## 8. Work breakdown

14 tasks. No two tasks own the same file — verified by the file lists below; where a
